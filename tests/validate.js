...
/* Content + contract validator. Catches the regressions unit tests can't:
   malformed game data, a game missing from the plan or the offline cache,
   and a stale service-worker hash (the stale-cache bug). Exits non-zero on
   any failure so CI fails loudly. */
const C = require("../shared/plan-core.js");
const { parsePrecache, readAssetsHash, computeAssetsHash, loadData, appSlugs } = require("./lib.js");
...
[["digraphs", 4], ["blends", 3], ["vowelTeams", 3]].forEach(([key, len]) => {
...
ok(data.phonicsCheck.some((w) => w.real) && data.phonicsCheck.some((w) => !w.real),
...
data.sightSets.forEach((set, s) => set.forEach((pair, i) => {
...
slugsOnDisk.forEach((slug) => {
  ok(slugsInPlan.includes(slug), `apps/${slug}.html exists but isn't in the plan catalogue (shared/plan-core.js GAMES)`);
});

["index.html", "parent.html", "manifest.webmanifest",
 "shared/pop.css", "shared/pop.js", "shared/data.js", "shared/plan-core.js", "shared/plan.js"
].forEach((f) => ok(precache.includes(f), `${f} is not in the service-worker PRECACHE`));

/* every game referenced by the plan must point at a real stage game too */
C.READING.concat(C.NUMBERS).forEach((stage) => stage.games.forEach((slug) => {
...
ok(stamped === actual,
  ...
  `A precached file changed — run \`npm run stamp\` (and bump VERSION) so the offline cache busts.`);
...
console.log("✓ validate: content, contract and service-worker cache all consistent (" +
  slugsInPlan.length + " games).");
