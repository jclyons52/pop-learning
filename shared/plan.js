...
(function (global) {
  ...
  function progress() {
    return (Pop.progress && Pop.progress.all()) || {};
  }
  function activity() {
    return load("pop-activity", {});
  }
  function overrides() {
    return load("pop-stage", {});
  }
  ...
  function todayPlan() {
    return Core.decoratePlan(rawPlan(), activity(), today());
  }
  function completion() {
  ...
  function isDone(slug) {
    return Core.isDone(activity(), slug, today());
  }
  function streak() {
    return Core.streakValue(load("pop-streak", null), today());
  }
  ...
  function checkComplete() {
    ...
    if (c.total > 0 && c.done >= c.total) {
      ...
      if (res.bumped) {
        save("pop-streak", res.state);
        return { justCompleted: true, streak: res.state.count };
      }
  ...
  function setStage(track, idxOrNull) {
    var o = overrides();
    if (idxOrNull == null) delete o[track];
    else o[track] = idxOrNull;
    save("pop-stage", o);
  ...
  Pop.plan = {
    ...
    stageInfo: stageInfo,
    setStage: setStage,
    stepsPerDay: Core.STEPS_PER_DAY,
  };
})(window);
