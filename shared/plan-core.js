...
(function () {
  ...
  function shift(dateStr, days) { var d = new Date(dateStr + "T00:00:00"); d.setDate(d.getDate()+days); return fmt(d); }
  ...
  function daysSince(activity, slug, today) { var lp = lastPlayed(activity, slug); return lp ? daysBetween(lp, today) : 999; }
  /** @param {ActivityMap} activity @param {string} slug */
  function daysPlayed(activity, slug) { var n = 0; for (var d in activity) if (activity[d][slug]) n++; return n; }
  /** @param {ActivityMap} activity @param {string} slug @param {string} today */
  function isDone(activity, slug, today) { var a = activity[today] || {}; return (a[slug] || 0) >= DONE_HITS; }
  ...
  function gameMastered(slug, progress, activity, today) {
    var g = GAMES[slug];
    if (!g) return false;
    if (g.kind === "recall") { var s = statsFor(progress, g.gid); return s.seen >= MASTER_SEEN && s.acc >= MASTER_ACC; }
  ...
  function stageComplete(stage, progress, activity, today) {
    for (var i = 0; i < stage.games.length; i++) if (!gameMastered(stage.games[i], progress, activity, today)) return false;
    return true;
  }
  ...
  function stageIndex(track, progress, activity, overrides, today) {
    ...
    for (var i = 0; i < stages.length; i++) if (!stageComplete(stages[i], progress, activity, today)) return i;
  ...
  function dueScore(slug, progress, activity, today, rng) {
    ...
    if (g && g.kind === "recall") { var s = statsFor(progress, g.gid); shaky = s.seen ? (1 - s.acc) : 0.5; }
  ...
  function buildPlan(state) {
    var progress = state.progress, activity = state.activity, overrides = state.overrides, today = state.today;
    ...
    function add(role, slug) { if (slug) { used[slug] = true; steps.push([slug, role]); } }
    ...
    /** @type {string[]} */
    var all = READING.concat(NUMBERS).reduce(function (a, s) { return a.concat(s.games); }, /** @type {string[]} */([]));
  ...
  function decoratePlan(plan, activity, today) {
    return plan.steps.map(function (p) {
      ...
      return { slug:slug, name: g ? g.name : slug, href:"apps/"+slug+".html",
        role:role, roleLabel:r.label, roleIcon:r.icon, done: isDone(activity, slug, today) };
    });
  }
  ...
  function stageInfoFor(track, progress, activity, overrides, today) {
    ...
    return { index: idx, name: stages[idx].name,
      auto: !overrides || overrides[track] == null, count: stages.length,
      names: stages.map(function (s) { return s.name; }) };
  }

  var api = {
    STEPS_PER_DAY: STEPS_PER_DAY, DONE_HITS: DONE_HITS, MASTER_SEEN: MASTER_SEEN, MASTER_ACC: MASTER_ACC,
    GAMES: GAMES, READING: READING, NUMBERS: NUMBERS, ROLES: ROLES,
    shift: shift, daysBetween: daysBetween,
    statsFor: statsFor, lastPlayed: lastPlayed, daysSince: daysSince, daysPlayed: daysPlayed, isDone: isDone,
    gameMastered: gameMastered, stageComplete: stageComplete, stageIndex: stageIndex,
    rngFrom: rngFrom, dueScore: dueScore, pickDue: pickDue,
    buildPlan: buildPlan, decoratePlan: decoratePlan,
